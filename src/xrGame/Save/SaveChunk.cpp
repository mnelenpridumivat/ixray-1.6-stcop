...
void CSaveChunk::r_vec3(Fvector& A)
{
	if (_currentArrayStack.empty()) {
		...
		A = SSaveVariableGetter::GetValue<Fvector, CSaveVariableVec3>(_variables[_currentReadIndex++].get());
	}
	else {
		auto CurrentArray = _currentArrayStack.top();
...
void CSaveChunk::r_vec4(Fvector4& A)
{
	if (_currentArrayStack.empty()) {
		...
		A = SSaveVariableGetter::GetValue<Fvector4, CSaveVariableVec4>(_variables[_currentReadIndex++].get());
	}
	else {
		auto CurrentArray = _currentArrayStack.top();
...
void CSaveChunk::r_float(float& A)
{
	if (_currentArrayStack.empty()) {
		...
		A = SSaveVariableGetter::GetValue<float, CSaveVariableFloat>(_variables[_currentReadIndex++].get());
	}
	else {
		auto CurrentArray = _currentArrayStack.top();
...
void CSaveChunk::r_double(double& A)
{
	if (_currentArrayStack.empty()) {
		...
		A = SSaveVariableGetter::GetValue<double, CSaveVariableDouble>(_variables[_currentReadIndex++].get());
	}
	else {
		auto CurrentArray = _currentArrayStack.top();
...
void CSaveChunk::r_u64(u64& A)
{
	if (_currentArrayStack.empty()) {
		...
		A = SSaveVariableGetter::GetValue<u64, CSaveVariableU64>(_variables[_currentReadIndex++].get());
	}
	else {
		auto CurrentArray = _currentArrayStack.top();
...
void CSaveChunk::r_s64(s64& A)
{
	if (_currentArrayStack.empty()) {
		...
		A = SSaveVariableGetter::GetValue<s64, CSaveVariableS64>(_variables[_currentReadIndex++].get());
	}
	else {
		auto CurrentArray = _currentArrayStack.top();
...
void CSaveChunk::r_u32(u32& A)
{
	if (_currentArrayStack.empty()) {
		...
		A = SSaveVariableGetter::GetValue<u32, CSaveVariableU32>(_variables[_currentReadIndex++].get());
	}
	else {
		auto CurrentArray = _currentArrayStack.top();
...
void CSaveChunk::r_s32(s32& A)
{
	if (_currentArrayStack.empty()) {
		...
		A = SSaveVariableGetter::GetValue<s32, CSaveVariableS32>(_variables[_currentReadIndex++].get());
	}
	else {
		auto CurrentArray = _currentArrayStack.top();
...
void CSaveChunk::r_u16(u16& A)
{
	if (_currentArrayStack.empty()) {
		...
		A = SSaveVariableGetter::GetValue<u16, CSaveVariableU16>(_variables[_currentReadIndex++].get());
	}
	else {
		auto CurrentArray = _currentArrayStack.top();
...
void CSaveChunk::r_s16(s16& A)
{
	if (_currentArrayStack.empty()) {
		...
		A = SSaveVariableGetter::GetValue<s16, CSaveVariableS16>(_variables[_currentReadIndex++].get());
	}
	else {
		auto CurrentArray = _currentArrayStack.top();
...
void CSaveChunk::r_u8(u8& A)
{
	if (_currentArrayStack.empty()) {
		...
		A = SSaveVariableGetter::GetValue<u8, CSaveVariableU8>(_variables[_currentReadIndex++].get());
	}
	else {
		auto CurrentArray = _currentArrayStack.top();
...
void CSaveChunk::r_s8(s8& A)
{
	if (_currentArrayStack.empty()) {
		...
		A = SSaveVariableGetter::GetValue<s8, CSaveVariableS8>(_variables[_currentReadIndex++].get());
	}
	else {
		auto CurrentArray = _currentArrayStack.top();
...
void CSaveChunk::r_stringZ(LPSTR S)
{
	if (_currentArrayStack.empty()) {
		...
		auto TempStr = SSaveVariableGetter::GetValue<xr_string, CSaveVariableBool>(_variables[_currentReadIndex++].get());
...
void CSaveChunk::r_stringZ(xr_string& dest)
{
	if (_currentArrayStack.empty()) {
		VERIFY(_variables[_currentReadIndex]->GetVariableType() == ESaveVariableType::t_string);
		dest = SSaveVariableGetter::GetValue<xr_string, CSaveVariableBool>(_variables[_currentReadIndex++].get());
	}
	else {
		auto CurrentArray = _currentArrayStack.top();
		VERIFY(CurrentArray->GetCurrentElement()->GetVariableType() == ESaveVariableType::t_string);
...
void CSaveChunk::r_stringZ(shared_str& dest)
{
	if (_currentArrayStack.empty()) {
		VERIFY(_variables[_currentReadIndex]->GetVariableType() == ESaveVariableType::t_string);
		dest = SSaveVariableGetter::GetValue<xr_string, CSaveVariableBool>(_variables[_currentReadIndex++].get()).c_str();
	}
	else {
		auto CurrentArray = _currentArrayStack.top();
		VERIFY(CurrentArray->GetCurrentElement()->GetVariableType() == ESaveVariableType::t_string);
...
void CSaveChunk::r_matrix(Fmatrix& M)
{
	if (_currentArrayStack.empty()) {
		...
		M = SSaveVariableGetter::GetValue<Fmatrix, CSaveVariableMatrix>(_variables[_currentReadIndex++].get());
	}
	else {
		auto CurrentArray = _currentArrayStack.top();
...
void CSaveChunk::r_clientID(ClientID& C)
{
	if (_currentArrayStack.empty()) {
		...
		C = SSaveVariableGetter::GetValue<ClientID, CSaveVariableClientID>(_variables[_currentReadIndex++].get());
	}
	else {
		auto CurrentArray = _currentArrayStack.top();
