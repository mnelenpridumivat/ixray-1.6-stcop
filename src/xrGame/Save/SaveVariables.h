...
public:
	CSaveVariableFloatQ16(float Value, float Min, float Max) : _value(Value), _min(Min), _max(Max) {}
...
public:
	CSaveVariableFloatQ8(float Value, float Min, float Max) : _value(Value), _min(Min), _max(Max) {}
...
struct SSaveVariableGetter {
	...
	static TType GetValue(CSaveVariableBase* Var) { return ((TVarClass*)Var)->_value; }
};