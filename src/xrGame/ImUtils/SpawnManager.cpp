...
float SpawnManager_ParseHitPower(const shared_str& hit_str) {
	...
	if (CSingleGameStats::GetInstance().GetSingleGameDifficulty() == egdNovice && num_game_diff_param > 2) {
		result = atof(_GetItem(*hit_str, 3, buffer));
	}
	else if (CSingleGameStats::GetInstance().GetSingleGameDifficulty() == egdStalker && num_game_diff_param > 1) {
		result = atof(_GetItem(*hit_str, 2, buffer));
	}
	else if (CSingleGameStats::GetInstance().GetSingleGameDifficulty() == egdVeteran && num_game_diff_param > 0) {
