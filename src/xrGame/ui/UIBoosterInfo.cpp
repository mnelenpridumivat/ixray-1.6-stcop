...
void CUIBoosterInfo::InitFromXml(CUIXml& xml)
{
	...
	for(u32 i = 0; i < eBoostExplImmunity; ++i)
	{
		if (xml.NavigateToNode(ef_boosters_section_names[i]))
		{
			...
			LPCSTR name = CStringTable::GetInstance().translate(boost_influence_caption[i]).c_str();
	...
	LPCSTR name = CStringTable::GetInstance().translate("ui_inv_satiety").c_str();
	...
	if (xml.NavigateToNode("boost_anabiotic"))
	{
		...
		name = CStringTable::GetInstance().translate("ui_inv_survive_surge").c_str();
	...
	if (xml.NavigateToNode("boost_time"))
	{
		...
		name = CStringTable::GetInstance().translate("ui_inv_effect_time").c_str();
