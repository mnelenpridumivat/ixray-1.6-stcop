...
void CUIArtefactParams::InitFromXml( CUIXml& xml )
{
	...
	if (xml.NavigateToNode("condition"))
	{
		...
		name = CStringTable::GetInstance().translate("st_condition").c_str();
	...
	for ( u32 i = 0; i < ALife::infl_max_count; ++i )
	{
		...
		name = CStringTable::GetInstance().translate(af_immunity_caption[i]).c_str();
	...
	for ( u32 i = 0; i < ALife::eRestoreTypeMax; ++i )
	{
		...
		name = CStringTable::GetInstance().translate(af_restore_caption[i]).c_str();
	...
	if (xml.NavigateToNode("af_slots"))
	{
		...
		name = CStringTable::GetInstance().translate("st_prop_artefact").c_str();
...

	{
		...
		name = CStringTable::GetInstance().translate("ui_inv_weight").c_str();
		LPCSTR add_name = CStringTable::GetInstance().translate("ui_inv_outfit_additional_weight").c_str();
