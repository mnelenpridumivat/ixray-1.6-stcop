...
SDrawStaticStruct * CUIGameCustom::AddHudMessage(LPCSTR text, LPCSTR text2, LPCSTR id, bool trnslate_second_text, float time, bool bSingleInstance) {
	...
	if (text2) {
		xr_sprintf(str, "%s : %s", *CStringTable::GetInstance().translate(text), trnslate_second_text ? *CStringTable::GetInstance().translate(text2) : text2);
	} else {
		xr_sprintf(str, "%s", *CStringTable::GetInstance().translate(text));
	}
