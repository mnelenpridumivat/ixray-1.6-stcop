...
#define ADD_BOOL_INFO(i, s, t, k)	i->m_aInfos.push_back(GameInfo(t, ((xrGS_SBServerGetBoolValueA(s, m_pQR2->xrGS_RegisteredKey(k), SBFalse)) == SBTrue)? *CStringTable::GetInstance().translate("mp_si_yes") : *CStringTable::GetInstance().translate("mp_si_no")))
...
{string256 tmp; xr_sprintf(tmp, "%d" t2, xrGS_SBServerGetIntValueA(s, m_pQR2->xrGS_RegisteredKey(k), 0)*m);\
	...
	else {i->m_aInfos.push_back(GameInfo(t1, *CStringTable::GetInstance().translate("mp_si_no")));}}
...
//	pServerInfo->m_aInfos.push_back(GameInfo("Version:", pServerInfo->m_ServerVersion));
	pServerInfo->m_aInfos.push_back(GameInfo(*CStringTable::GetInstance().translate("mp_si_servername"), pServerInfo->m_ServerName));
	pServerInfo->m_aInfos.push_back(GameInfo(*CStringTable::GetInstance().translate("mp_si_version"), pServerInfo->m_ServerVersion));
	
	ADD_INT_INFO_N(pServerInfo, pServer, 1, *CStringTable::GetInstance().translate("mp_si_max_ping"), "", G_MAX_PING_KEY);
	ADD_BOOL_INFO(pServerInfo, pServer, *CStringTable::GetInstance().translate("mp_si_maprotation"), G_MAP_ROTATION_KEY);
	
	pServerInfo->m_aInfos.push_back(
		GameInfo(*CStringTable::GetInstance().translate("mp_si_voting"),
		(xrGS_SBServerGetBoolValueA(pServer, m_pQR2->xrGS_RegisteredKey(G_VOTING_ENABLED_KEY), SBFalse) == SBTrue) ?
			*CStringTable::GetInstance().translate("mp_si_enabled") : *CStringTable::GetInstance().translate("mp_si_disabled")));

//	ADD_BOOL_INFO(pServerInfo, pServer, *g_pStringTable->translate("mp_si_voting"), G_VOTING_ENABLED_KEY);
	//-----------------------------------------------------------------------
	pServerInfo->m_aInfos.push_back(GameInfo(*CStringTable::GetInstance().translate("mp_si_spectatormodes"), ""));
	int SpectrModes = xrGS_SBServerGetIntValueA(pServer, m_pQR2->xrGS_RegisteredKey(G_SPECTATOR_MODES_KEY), 0);
	
	pServerInfo->m_aInfos.push_back(GameInfo(*CStringTable::GetInstance().translate("mp_si_free_fly"), ((SpectrModes & (1<<CSpectator::eacFreeFly	)) != 0) ? *CStringTable::GetInstance().translate("mp_si_yes") : *CStringTable::GetInstance().translate("mp_si_no")));
	pServerInfo->m_aInfos.push_back(GameInfo(*CStringTable::GetInstance().translate("mp_si_first_eye"), ((SpectrModes & (1<<CSpectator::eacFirstEye	)) != 0) ? *CStringTable::GetInstance().translate("mp_si_yes") : *CStringTable::GetInstance().translate("mp_si_no")));
	pServerInfo->m_aInfos.push_back(GameInfo(*CStringTable::GetInstance().translate("mp_si_look_at"), ((SpectrModes & (1<<CSpectator::eacLookAt	)) != 0) ? *CStringTable::GetInstance().translate("mp_si_yes") : *CStringTable::GetInstance().translate("mp_si_no")));
	pServerInfo->m_aInfos.push_back(GameInfo(*CStringTable::GetInstance().translate("mp_si_free_look"), ((SpectrModes & (1<<CSpectator::eacFreeLook	)) != 0) ? *CStringTable::GetInstance().translate("mp_si_yes") : *CStringTable::GetInstance().translate("mp_si_no")));
	if (pServerInfo->m_GameType != eGameIDDeathmatch)
		pServerInfo->m_aInfos.push_back(GameInfo(*CStringTable::GetInstance().translate("mp_si_team_only"), ((SpectrModes & (1<<CSpectator::eacMaxCam	)) != 0) ? *CStringTable::GetInstance().translate("mp_si_yes") : *CStringTable::GetInstance().translate("mp_si_no")));
	//-----------------------------------------------------------------------
	
	if (pServerInfo->m_GameType == eGameIDDeathmatch || pServerInfo->m_GameType == eGameIDTeamDeathmatch) 
	{
		ADD_INT_INFO_N (pServerInfo, pServer, 1, *CStringTable::GetInstance().translate("mp_si_fraglimit"), "", G_FRAG_LIMIT_KEY);	
	}

	ADD_TIME_INFO(pServerInfo, pServer, 1.0f, *CStringTable::GetInstance().translate("mp_si_time_limit"), "%.0f %s",*CStringTable::GetInstance().translate("mp_si_min"), G_TIME_LIMIT_KEY);

	if (xrGS_SBServerGetIntValueA(pServer, m_pQR2->xrGS_RegisteredKey(G_DAMAGE_BLOCK_TIME_KEY), 0) != 0)
	{
		pServerInfo->m_aInfos.push_back(GameInfo(*CStringTable::GetInstance().translate("mp_si_invinsibility"), ""));
		ADD_BOOL_INFO(pServerInfo, pServer, *CStringTable::GetInstance().translate("mp_si_invinsibility_indicators"), G_DAMAGE_BLOCK_INDICATOR_KEY);
		ADD_TIME_INFO(pServerInfo, pServer, 1.0f, *CStringTable::GetInstance().translate("mp_si_invinsibility_time"), "%.f %s",*CStringTable::GetInstance().translate("mp_si_sec"), G_DAMAGE_BLOCK_TIME_KEY);
	}	

	ADD_BOOL_INFO(pServerInfo, pServer, *CStringTable::GetInstance().translate("mp_si_anomalies"), G_ANOMALIES_ENABLED_KEY);
	if ((xrGS_SBServerGetBoolValueA(pServer, m_pQR2->xrGS_RegisteredKey(G_ANOMALIES_ENABLED_KEY), SBFalse)) == SBTrue)
	{
		...
		{
			ADD_TIME_INFO(pServerInfo, pServer, 1.0f, *CStringTable::GetInstance().translate("mp_si_anomalies_period"), "%.1f %s",*CStringTable::GetInstance().translate("mp_si_min"), G_ANOMALIES_TIME_KEY);
		}
		else
			pServerInfo->m_aInfos.push_back(GameInfo(*CStringTable::GetInstance().translate("mp_si_anomalies_period"), *CStringTable::GetInstance().translate("mp_si_infinite")));
	}

	ADD_TIME_INFO(pServerInfo, pServer, 1.0f, *CStringTable::GetInstance().translate("mp_si_forcerespawn"), "%.f %s",*CStringTable::GetInstance().translate("mp_si_sec"), G_FORCE_RESPAWN_KEY);
	ADD_TIME_INFO(pServerInfo, pServer, 1.0f, *CStringTable::GetInstance().translate("mp_si_warmuptime"), "%.0f %s",*CStringTable::GetInstance().translate("mp_si_sec"), G_WARM_UP_TIME_KEY);
	...
	{
		ADD_BOOL_INFO(pServerInfo, pServer, *CStringTable::GetInstance().translate("mp_si_autoteam_balance"), G_AUTO_TEAM_BALANCE_KEY);
		ADD_BOOL_INFO(pServerInfo, pServer, *CStringTable::GetInstance().translate("mp_si_autoteam_swap"), G_AUTO_TEAM_SWAP_KEY);
		ADD_BOOL_INFO(pServerInfo, pServer, *CStringTable::GetInstance().translate("mp_si_friendly_indicators"), G_FRIENDLY_INDICATORS_KEY);
		ADD_BOOL_INFO(pServerInfo, pServer, *CStringTable::GetInstance().translate("mp_si_friendly_names"), G_FRIENDLY_NAMES_KEY);

		ADD_INT_INFO_N (pServerInfo, pServer, 1/100.0f, *CStringTable::GetInstance().translate("mp_si_friendly_fire"), " %%", G_FRIENDLY_FIRE_KEY);
	};

	if (pServerInfo->m_GameType == eGameIDArtefactHunt || pServerInfo->m_GameType == eGameIDCaptureTheArtefact)
	{
		pServerInfo->m_aInfos.push_back(GameInfo(*CStringTable::GetInstance().translate("mp_si_artefacts"), ""));
		ADD_INT_INFO(pServerInfo, pServer, *CStringTable::GetInstance().translate("mp_si_afcount"), G_ARTEFACTS_COUNT_KEY);

		ADD_TIME_INFO(pServerInfo, pServer, 1.0f, *CStringTable::GetInstance().translate("mp_si_afstaytime"), "%.2f %s",*CStringTable::GetInstance().translate("mp_si_min"), G_ARTEFACT_STAY_TIME_KEY);
		ADD_TIME_INFO(pServerInfo, pServer, 1.0f, *CStringTable::GetInstance().translate("mp_si_afrespawntime"), "%.0f %s",*CStringTable::GetInstance().translate("mp_si_sec"), G_ARTEFACT_RESPAWN_TIME_KEY);

		int Reinforcement = atoi(xrGS_SBServerGetStringValueA(pServer, m_pQR2->xrGS_RegisteredKey(G_REINFORCEMENT_KEY), "0"));		
		switch (Reinforcement)
		{
		case -1:
			pServerInfo->m_aInfos.push_back(GameInfo(*CStringTable::GetInstance().translate("mp_si_players_respawn"), *CStringTable::GetInstance().translate("mp_si_artefact_captured")));
			break;
		case 0:
			pServerInfo->m_aInfos.push_back(GameInfo(*CStringTable::GetInstance().translate("mp_si_players_respawn"), *CStringTable::GetInstance().translate("mp_si_any_time")));
			break;
		default:
			ADD_TIME_INFO(pServerInfo, pServer, 1.0f, *CStringTable::GetInstance().translate("mp_si_players_respawn"), "%.0f %s",*CStringTable::GetInstance().translate("mp_si_sec"), G_REINFORCEMENT_KEY);
			break;
		}

		ADD_BOOL_INFO(pServerInfo, pServer, *CStringTable::GetInstance().translate("mp_si_shielded_bases"),					G_SHIELDED_BASES_KEY	);
		ADD_BOOL_INFO(pServerInfo, pServer, *CStringTable::GetInstance().translate("mp_si_return_players"),					G_RETURN_PLAYERS_KEY	);
		ADD_BOOL_INFO(pServerInfo, pServer, *CStringTable::GetInstance().translate("mp_si_afbearer_cant_sprint"),			G_BEARER_CANT_SPRINT_KEY);
	}
