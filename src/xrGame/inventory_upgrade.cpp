...
void Upgrade::construct( const shared_str& upgrade_id, Group& parental_group, Manager& manager_r )
{
	...
	m_name        = CStringTable::GetInstance().translate( pSettings->r_string(id(), "name") );
	m_description = CStringTable::GetInstance().translate( pSettings->r_string(id(), "description") );
