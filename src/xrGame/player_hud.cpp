...
float player_hud::CalcMotionSpeed(const shared_str& anim_name)
{
...
void player_hud::load(const shared_str& player_hud_sect)
{
	...
	if(l_arm != BI_NONE) {
		m_model->dcast_PKinematics()->LL_GetBoneInstance(l_arm).set_callback(bctCustom, [](CBoneInstance* B) {((player_hud*)g_player_hud)->LeftArmCallback(B); }, NULL);
	}
...
const shared_str& player_hud::section_name() const { return m_sect_name; }

attachable_hud_item* player_hud::attached_item(u16 item_idx) { return m_attached_items[item_idx]; }

void player_hud::detach_all_items() { m_attached_items[0] = NULL; m_attached_items[1] = NULL; }
