...
{ 
	auto E = 	physics_impl().get_Element( bone_name );
	if( !E )
	{
		return nullptr;
	}
...
{
	auto E = 	physics_impl().get_Element( bone_id );
	if( !E )
	{
		return nullptr;
	}
...
{
	auto E = physics_impl().get_ElementByStoreOrder( idx );
	R_ASSERT( E );
	return get_script_wrapper<cphysics_element_scripted>( *E ) ;
}
...
{
	auto J = 	physics_impl().get_Joint( bone_name );
	if(!J)
	{
		return nullptr;
	}
...
{
	auto J = physics_impl().get_Joint( bone_id );
	if(!J)
	{
		return nullptr;
	}
...
{
	auto J = physics_impl().get_JointByStoreOrder( idx );
	R_ASSERT( J );
	return get_script_wrapper<cphysics_joint_scripted>( *J ) ;
}
