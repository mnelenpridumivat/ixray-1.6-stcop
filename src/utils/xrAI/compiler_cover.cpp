...
IC float getLastRP_Scale(const xrPhysX::CDB::TraceResult& result, RayCache& C)
{
	//u32	tris_count		= DB->r_count();
	float	scale		= 1.f;
	Fvector B;

	for (const auto& rpinf : result.results)
	{
...
IC float rayTrace	(const xrPhysX::CDB::RayTraceOptions& options, Fvector& P, Fvector& D, float R, RayCache& C)
{
	//R_ASSERT	(DB);
	...
	LevelPtr->RayTrace(opt_copy, result);
	...
	if (result.results.empty()) {
		...
		return getLastRP_Scale(result,C);
	}
//	return 0;
}
...
public:
	...
	void				compute_cover_value	(u32 const &N, vertex &BaseNode, float const &cover_height, Cover &cover)
	{
		...
		for (Nearest_it it=Q.q_List.begin(); it!=Q.q_List.end();  it++)
		{
			...
			c_total		[sector]	+=	1.f;
			c_passed	[sector]	+=	rayTrace (options, TestPos, Dir, range, cache[ID].C); //
		}
		Q.Clear			();
