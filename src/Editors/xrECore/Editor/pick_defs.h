...
class CCustomObject;
	...
	class SPickQuery{
    public:
    	xrPhysX::CDB::TraceOptions m_Flags;
        Fvector				m_Start;
        Fvector				m_Direction;
        float				m_Dist;
        Fbox 				m_BB; 
...
//.            u32				tag;
    ...
    protected:
    	ResultVec			results;
    public:
        IC void	prepare_rq	(const Fvector& start, const Fvector& dir, float dist)
        {
            m_Start.set		(start);
            m_Direction.set	(dir);
            m_Dist			= dist;
        ...
        IC void	prepare_bq	(const Fbox& bbox)
        {
        	m_BB.set		(bbox);
        	results.clear	();
        }
        ...
        {
        	SResult	D(parent, R, obj, mesh);
        	if ((bool)(m_Flags&xrPhysX::CDB::TraceOptions::only_nearest)&&!results.empty()){
        		SResult& S	= results.back();
        		if (D.range<S.range)
        		{
        			S = D;
