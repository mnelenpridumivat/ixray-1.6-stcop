...
void ESceneGroupTool::ReloadRefsSelectedObject()
{
    ...
    if (!lst.empty())
    {
        ...
        GetTempFileNameA			( FS.get_path(_temp_)->m_Path.xstring().c_str(), "tmp_sector", 0, temp_file_name_sector );
        Scene->SaveToolLTX		(OBJCLASS_SECTOR, temp_file_name_sector);

        GetTempFileNameA			( FS.get_path(_temp_)->m_Path.xstring().c_str(), "tmp_portal", 0, temp_file_name_portal );
