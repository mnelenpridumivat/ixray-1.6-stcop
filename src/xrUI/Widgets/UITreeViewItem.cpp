...
void CreateTreeBranch(shared_str nesting, shared_str leafName, CUIListWnd* pListToAdd, int leafProperty, CGameFont* pRootFont, u32 rootColor, CGameFont* pLeafFont, u32 leafColor, bool markRead)
{
	...
	public:
		...
		CUITreeViewItem* operator () (GroupTree_it it, GroupTree& cont, CUITreeViewItem* pItemToIns)
		{
			...
			for (GroupTree_it it2 = it; it2 != cont.end(); ++it2)
			{
				...
				pNewItem->TextItemControl()->SetTextAlignment(ETextAlignment::alLeft);
	...
	while (true)
	{
		...
		if (pos != xr_string::npos)
		{
			...
			groupTree.push_back(CStringTable::GetInstance().translate(str));
			group.erase(0, pos + 1);
		}
		else
		{
			groupTree.push_back(CStringTable::GetInstance().translate(group.c_str()));
			break;
		}
	}
	...
	if (!pTVItemChilds)
	{
		...
		pTVItemChilds->TextItemControl()->SetTextAlignment(ETextAlignment::alLeft);
	...
	pTVItem->SetText(*CStringTable::GetInstance().translate(*leafName));
	pTVItem->SetValue(leafProperty);
	pTVItem->TextItemControl()->SetTextAlignment(ETextAlignment::alLeft);
